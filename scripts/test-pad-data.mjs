...
import { decodeIosc, decodePadTexturePixels, PadDataArchive, parsePadTexture, parseTex1, parseTex2 } from '../src/binary-port/padDataArchive.js';
...
const monster1 = parseTex1(monsterTexture);
assert.deepEqual([monster1.width, monster1.height, monster1.formatFlags, monster1.bytesPerPixel], [256, 256, 0x3000, 2]);
assert.equal(monster1.sourceName, 'MONS_001.PNG');
const monster1Rgba = decodePadTexturePixels(monster1);
assert.equal(monster1Rgba.length, 256 * 256 * 4);
assert.deepEqual(Array.from(monster1Rgba.subarray(0, 4)), [255, 255, 255, 0]);
const monster147 = parsePadTexture(archive.read('mons_147.btex', inflateBytes));
assert.deepEqual([monster147.width, monster147.height, monster147.formatFlags, monster147.bytesPerPixel], [256, 256, 0x3000, 2]);
assert.equal(monster147.sourceName, 'MONS_147.PNG');
assert.equal(monster147.sprites.length, 1);
assert.equal(decodePadTexturePixels(monster147).length, 256 * 256 * 4);
assert.equal(archive.read('missing.btex'), null);
const badChecksum = compressedBytes.slice();
badChecksum[6] ^= 1;
assert.throws(() => decodeIosc(badChecksum, inflateBytes), /CRC mismatch/);
console.log('PAD data index, resident streams, IOSC decoding, TEX1/TEX2 textures, and packed pixel checks passed.');
