...
const renderBoardSizeChangeState = process.argv.includes('--board-size-change-render');
const renderNoSkyfallState = process.argv.includes('--no-skyfall-render');
...
try {
  ...
  const remainingEnemiesTurnChangeRenderState = renderRemainingEnemiesTurnChangeState
    ? await page.evaluate(() => {
      const engine = window.__puzzleGame;
      const monsterDefinition = new Uint8Array(0x2ec);
      const monsterView = new DataView(monsterDefinition.buffer);
      ...
      monsterView.setInt16(0xe2, 100, true);
      monsterView.setInt16(0xe4, 10, true);
      monsterView.setUint32(0xec, 9_122, true);
      monsterView.setUint8(0xf0, 100);
      const definition = new Uint8Array(0x48);
      const view = new DataView(definition.buffer);
      view.setUint32(0x00, 9_122, true);
      view.setInt16(0x04, 122, true);
      view.setInt16(0x10, 1, true);
      view.setInt16(0x14, 1, true);
      view.setInt32(0x30, 10_000, true);
      view.setInt32(0x34, 1_000, true);
      view.setInt32(0x38, 100, true);
      view.setInt32(0x40, 20, true);
  ...
  if (remainingEnemiesTurnChangeRenderState && (
    remainingEnemiesTurnChangeRenderState.lastDamage !== 1_660
    || remainingEnemiesTurnChangeRenderState.enemies?.[0]?.hp !== 90_340
    || remainingEnemiesTurnChangeRenderState.enemies?.[0]?.remainingEnemiesTurnChangeThreshold !== 1
    || remainingEnemiesTurnChangeRenderState.enemies?.[0]?.remainingEnemiesTurnChangeCounter !== 1
    || remainingEnemiesTurnChangeRenderState.enemies?.[0]?.remainingEnemiesTurnChangeActive !== true
    || remainingEnemiesTurnChangeRenderState.enemies?.[0]?.maxCounter !== 1
    || remainingEnemiesTurnChangeRenderState.enemies?.[0]?.counter !== 1
    || remainingEnemiesTurnChangeRenderState.rngState !== 21_900
  )) throw new Error(
    `Remaining-enemies-turn-change render-state mismatch: ${JSON.stringify(remainingEnemiesTurnChangeRenderState)}`,
  );
  if (remainingEnemiesTurnChangeRenderState) await page.evaluate(() => new Promise(requestAnimationFrame));
  ...
  const boardSizeChangeRenderState = renderBoardSizeChangeState ? await page.evaluate(() => {
    const engine = window.__puzzleGame;
    engine.reset();
    engine.start();
    engine.setBoardFromCodes([
    ...
    const preservedOrb = engine.board[4][5];
    engine.setRngState(21_900);
    const applied = engine.applyEnemySkillRecord({
      type: 126,
    ...
    engine.advanceBoardSizeChangeTurnsPostEnemyAttack();
    const afterFreshBoundary = engine.snapshot();
    for (let index = 0; index < 7; index += 1) {
      engine.advanceBoardSizeChangeTurnsPostEnemyAttack();
    }
    const restored = engine.snapshot();
    const preservedOrbRestored = engine.board[4][5] === preservedOrb;
    // Leave the visual checkpoint on the active compact layout so the
    // screenshot also exercises the dynamic board renderer and status label.
    engine.applyEnemySkillRecord({
      type: 126,
      kind: 'boardSizeChange',
      supported: true,
      durationTurns: 7,
    ...
    return {
      ...
      afterFreshBoundary,
      restored,
      preservedOrbRestored,
    };
  }) : null;
  if (boardSizeChangeRenderState && (
    boardSizeChangeRenderState.applied !== true
    || boardSizeChangeRenderState.active?.boardDimensions?.rows !== 4
    || boardSizeChangeRenderState.active?.boardDimensions?.columns !== 5
    || boardSizeChangeRenderState.active?.boardSizeCode !== 0x45
    || boardSizeChangeRenderState.active?.boardSizeChange?.turnsRemaining !== 7
    || boardSizeChangeRenderState.afterFreshBoundary?.boardSizeChange?.turnsRemaining !== 7
    || boardSizeChangeRenderState.restored?.boardDimensions?.rows !== 5
    || boardSizeChangeRenderState.restored?.boardDimensions?.columns !== 6
    || boardSizeChangeRenderState.restored?.boardSizeChange !== null
    || boardSizeChangeRenderState.preservedOrbRestored !== true
  )) throw new Error(`Board-size-change render-state mismatch: ${JSON.stringify(boardSizeChangeRenderState)}`);
  if (boardSizeChangeRenderState) await page.evaluate(() => new Promise(requestAnimationFrame));
  const noSkyfallRenderState = renderNoSkyfallState ? await page.evaluate(() => {
    const engine = window.__puzzleGame;
    engine.reset();
    engine.start();
    engine.setBoardFromCodes([
    ...
    engine.startDrag(0, 0);
    engine.endDrag();
    engine.advancePhase();
    engine.advancePhase();
    engine.advancePhase();
    const active = engine.snapshot();
    engine.advanceNoSkyfallTurns();
    const afterBoundary = engine.snapshot();
    return { applied, active, afterBoundary };
  }) : null;
  if (noSkyfallRenderState && (
    noSkyfallRenderState.applied !== true
    || noSkyfallRenderState.active?.phase !== 'attack'
    || noSkyfallRenderState.active?.comboCount !== 1
    || noSkyfallRenderState.active?.board?.length !== 5
    || noSkyfallRenderState.active?.board?.some((row) => row.length !== 6)
    || noSkyfallRenderState.active?.noSkyfallRule?.active !== true
    || noSkyfallRenderState.active?.noSkyfallRule?.turnsRemaining !== 3
    || noSkyfallRenderState.afterBoundary?.noSkyfallRule?.turnsRemaining !== 2
  )) throw new Error(`No-skyfall render-state mismatch: ${JSON.stringify(noSkyfallRenderState)}`);
  if (noSkyfallRenderState) await page.evaluate(() => new Promise(requestAnimationFrame));
  ...
  const damageImmunityOffRenderState = renderDamageImmunityOffState ? await page.evaluate(() => {
    const engine = window.__puzzleGame;
    const monsterDefinition = new Uint8Array(0x2ec);
    const monsterView = new DataView(monsterDefinition.buffer);
    monsterView.setUint8(0xe0, 1);
    monsterView.setInt16(0xe2, 100, true);
    monsterView.setInt16(0xe4, 10, true);
    monsterView.setUint32(0xec, 9_121, true);
    monsterView.setUint8(0xf0, 100);
    const definition = new Uint8Array(0x48);
    const view = new DataView(definition.buffer);
    view.setUint32(0x00, 9_121, true);
    view.setInt16(0x04, 121, true);
    view.setInt32(0x30, 10_000, true);
    view.setInt32(0x34, 1_000, true);
    view.setInt32(0x38, 100, true);
    view.setInt32(0x40, 20, true);
    ...
    engine.setEnemySkillQueue(0, []);
    engine.setEnemyAiDefinitionPool(0, monsterDefinition, [definition]);
    engine.enemies[0].damageImmunityTurns = 3;
    engine.enemies[0].counter = 1;
    engine.enemies[1].counter = 99;
    engine.setRngState(21_900);
    engine.resolveEnemyTurn();
  ...
  if (damageImmunityOffRenderState && (
    damageImmunityOffRenderState.lastEnemyActions?.[0]?.skill?.type !== 121
    || damageImmunityOffRenderState.enemies?.[0]?.damageImmunityTurns !== 0
    || damageImmunityOffRenderState.enemies?.[0]?.hp !== 88_052
    || damageImmunityOffRenderState.lastDamage !== 3_948
    || damageImmunityOffRenderState.rngState !== 394_448_415
  )) throw new Error(
    `Damage-immunity-off render-state mismatch: ${JSON.stringify(damageImmunityOffRenderState)}`,
  );
  if (damageImmunityOffRenderState) await page.evaluate(() => new Promise(requestAnimationFrame));
  ...
  const legacyAiRenderState = renderLegacyAiState ? await page.evaluate(() => {
    const engine = window.__puzzleGame;
    const monsterDefinition = new Uint8Array(0x2ec);
    const monsterView = new DataView(monsterDefinition.buffer);
    // Clear mode bit 0 to select cGAMEMAIN::_chooseEnemyAi.
    monsterView.setUint8(0xe0, 0);
    monsterView.setInt16(0xe2, 100, true);
    monsterView.setInt16(0xe4, 10, true);
    monsterView.setInt16(0xe6, 100, true);
    monsterView.setUint32(0xec, 9_001, true);
    monsterView.setUint8(0xf0, 100);
    const definition = new Uint8Array(0x48);
    const view = new DataView(definition.buffer);
    view.setUint32(0x00, 9_001, true);
    view.setInt16(0x04, 128, true);
    view.setInt32(0x10, 3, true);
    view.setInt32(0x14, 75, true);
    view.setInt32(0x30, 10_000, true);
    view.setInt32(0x34, 1_000, true);
    view.setInt32(0x38, 100, true);
    view.setInt32(0x3c, 1_000, true);
    view.setInt32(0x40, 20, true);
    view.setInt32(0x44, 0, true);
    engine.reset();
    engine.start();
  ...
  if (legacyAiRenderState && (
    legacyAiRenderState.lastEnemyActions?.[0]?.skill?.type !== 128
    || legacyAiRenderState.lastEnemyActions?.[0]?.damage !== undefined
    || legacyAiRenderState.player?.hp !== 12_000
    || legacyAiRenderState.rngState !== 394_448_415
    || legacyAiRenderState.enemies?.[0]?.enemyAiBudget !== 80
    || legacyAiRenderState.enemies?.[0]?.aiUseCount !== 1
    || legacyAiRenderState.blackFallRule?.active !== true
