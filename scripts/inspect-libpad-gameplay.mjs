...
const LEGACY_FALLBACK_CLEAR_COUNT_TYPES = Object.freeze([6]);
...
const LEGACY_FALLBACK_INSTRUCTION_ANCHORS = Object.freeze([
  ...
  [0x61e448, 0xf9402be8], // type 57/59 face-list base
  [0x61e4d0, 0xb94016d7], // type 60/61 optional-Heart parameter
  [0x61e5d8, 0x34fff349], // type 60/61 positive-count gate
...
const RANDOM_BOMBS_ENEMY_SKILL_TYPE = 102;
const RANDOM_BOMBS_HANDLER = 0x62a0f0;
const RANDOM_BOMBS_SETUP_HANDLER = 0x61ffa8;
const RANDOM_BOMBS_CONDITION_HANDLER = 0x61a630;
const RANDOM_BOMBS_INSTRUCTION_ANCHORS = Object.freeze([
  [0x61ffa8, 0xf9400048], // setup addresses the shared game-work RNG owner
  [0x61ffb8, 0xb869690a], // load the current shared LCG state
  [0x61ffc8, 0x1b0b314a], // advance it with the native multiply-add constants
  [0x61ffcc, 0xb829690a], // persist the advanced shared state
  [0x61ffd0, 0x53107d48], // keep the high 16 bits as the private selection seed
  [0x61ffd4, 0xb9000348], // store that seed at runtime +0x678
  [0x61a630, 0x52a7f008], // condition returns the unconditional-admission sentinel
  [0x62a0f0, 0xb9402ea8], // execution loads the distant +0x2c locked-bomb flag
  [0x62a0f4, 0xb9467a61], // load prepared private selection seed at +0x678
  [0x62a0f8, 0xb94016a4], // load authored bomb count at +0x14
  [0x62a104, 0x1a9f07e2], // normalize the authored lock flag to boolean
...
const DAMAGE_BRANCH_ENEMY_SKILL_TYPE = 116;
const DAMAGE_BRANCH_HANDLER = 0x62be50;
const DAMAGE_BRANCH_SETUP_HANDLER = 0x621c94;
const DAMAGE_BRANCH_CONDITION_HANDLER = 0x61c01c;
const DAMAGE_BRANCH_INSTRUCTION_ANCHORS = Object.freeze([
  [0x621c94, 0x12800008], // setup writes selected-skill sentinel -1
  [0x621c98, 0xb9067268], // persist sentinel at sMONSTER+0x670
  [0x621c9c, 0x1e3e1000], // return setup scale -1.0 for the control record
  [0x62be50, 0x900045e9], // ordinary execution enters the generic inert tail
...
if (!inputPath || restoredFlag >= 0 && !restoredPath) {
  ...
  const legacyFallbackJumpTableMatches = restoredElf === null ? null
    : [
      ...
      ...LEGACY_FALLBACK_CLEAR_COUNT_TYPES,
      ...
      const expectedTarget = LEGACY_FALLBACK_SCALE_ZERO_TYPES.includes(type)
        ...
        : LEGACY_FALLBACK_SCALE_ONE_TYPES.includes(type)
          ...
          : LEGACY_FALLBACK_COMMON_ONE_TYPES.includes(type)
            ...
            : LEGACY_FALLBACK_EFFECTIVE_ONE_TYPES.includes(type)
              ? 0x61e9b8
              : LEGACY_FALLBACK_CLEAR_COUNT_TYPES.includes(type)
                ? 0x61e7d0
  ...
  const randomBombsDispatchTarget = resolveEnemySkillTarget(
    RANDOM_BOMBS_ENEMY_SKILL_TYPE, ENEMY_SKILL_DISPATCH_TABLE, ENEMY_SKILL_DISPATCH_BASE,
  );
  const randomBombsSetupTarget = resolveEnemySkillTarget(
    RANDOM_BOMBS_ENEMY_SKILL_TYPE, ENEMY_SKILL_SETUP_TABLE, ENEMY_SKILL_SETUP_BASE,
  );
  const randomBombsConditionTarget = resolveEnemySkillTarget(
    RANDOM_BOMBS_ENEMY_SKILL_TYPE, ENEMY_SKILL_CONDITION_TABLE, ENEMY_SKILL_CONDITION_BASE,
  );
  const randomBombsDispatchMatches = randomBombsDispatchTarget === null
    ? null : randomBombsDispatchTarget === RANDOM_BOMBS_HANDLER;
  const randomBombsSetupMatches = randomBombsSetupTarget === null
    ? null : randomBombsSetupTarget === RANDOM_BOMBS_SETUP_HANDLER;
  const randomBombsConditionMatches = randomBombsConditionTarget === null
    ? null : randomBombsConditionTarget === RANDOM_BOMBS_CONDITION_HANDLER;
  const randomBombsInstructionAnchorsMatch = restoredElf === null ? null
  ...
  const damageBranchDispatchTarget = resolveEnemySkillTarget(
    DAMAGE_BRANCH_ENEMY_SKILL_TYPE,
    ENEMY_SKILL_DISPATCH_TABLE,
    ENEMY_SKILL_DISPATCH_BASE,
  );
  const damageBranchSetupTarget = resolveEnemySkillTarget(
    DAMAGE_BRANCH_ENEMY_SKILL_TYPE,
    ENEMY_SKILL_SETUP_TABLE,
    ENEMY_SKILL_SETUP_BASE,
  );
  const damageBranchConditionTarget = resolveEnemySkillTarget(
    DAMAGE_BRANCH_ENEMY_SKILL_TYPE,
    ENEMY_SKILL_CONDITION_TABLE,
    ENEMY_SKILL_CONDITION_BASE,
  );
  const damageBranchDispatchMatches = damageBranchDispatchTarget === null
    ? null : damageBranchDispatchTarget === DAMAGE_BRANCH_HANDLER;
  const damageBranchSetupMatches = damageBranchSetupTarget === null
    ? null : damageBranchSetupTarget === DAMAGE_BRANCH_SETUP_HANDLER;
  const damageBranchConditionMatches = damageBranchConditionTarget === null
    ? null : damageBranchConditionTarget === DAMAGE_BRANCH_CONDITION_HANDLER;
  const damageBranchInstructionAnchorsMatch = restoredElf === null ? null
  ...
  const report = {
    ...
    restoration: restoredElf ? {
      ...
      randomBombsDispatchMatches21_9: randomBombsDispatchMatches,
      ...
      damageBranchDispatchMatches21_9: damageBranchDispatchMatches,
    ...
    enemySkillRuntime: {
      ...
      randomBombsType: RANDOM_BOMBS_ENEMY_SKILL_TYPE,
      randomBombsDispatchTarget: randomBombsDispatchTarget === null
        ? null : hex(randomBombsDispatchTarget),
      randomBombsDispatchMatches21_9: randomBombsDispatchMatches,
      randomBombsSetupTarget: randomBombsSetupTarget === null
        ? null : hex(randomBombsSetupTarget),
      randomBombsSetupMatches21_9: randomBombsSetupMatches,
      randomBombsConditionTarget: randomBombsConditionTarget === null
        ? null : hex(randomBombsConditionTarget),
      randomBombsConditionMatches21_9: randomBombsConditionMatches,
      randomBombsInstructionAnchorsMatch21_9: randomBombsInstructionAnchorsMatch,
      ...
      damageBranchType: DAMAGE_BRANCH_ENEMY_SKILL_TYPE,
      damageBranchDispatchTarget: damageBranchDispatchTarget === null
        ? null : hex(damageBranchDispatchTarget),
      damageBranchDispatchMatches21_9: damageBranchDispatchMatches,
      damageBranchSetupTarget: damageBranchSetupTarget === null
        ? null : hex(damageBranchSetupTarget),
      damageBranchSetupMatches21_9: damageBranchSetupMatches,
      damageBranchConditionTarget: damageBranchConditionTarget === null
        ? null : hex(damageBranchConditionTarget),
      damageBranchConditionMatches21_9: damageBranchConditionMatches,
      damageBranchInstructionAnchorsMatch21_9: damageBranchInstructionAnchorsMatch,
    ...
    enemyAi: {
      ...
      legacyFallbackClearCountTypes: LEGACY_FALLBACK_CLEAR_COUNT_TYPES,
  ...
  if (
    ...
    || randomBombsDispatchMatches === false
    ...
    || damageBranchDispatchMatches === false
